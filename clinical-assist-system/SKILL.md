...
- **重建后验证（session 31 收尾已完成）**：3171块/cosine/检索胆囊查询 dist=0.307 命中胆石症共识/absolute local_url ✓；rag-service 重启已批准执行，health `guidelines:3171` OK，需求1+2 代码随重启载入。**过渡态须知**：集合 delete→重建完成前的窗口期 `/health` 报 `{"status":"error","detail":"Collection [...] does not exist"}` 是预期现象（其余集合查询正常），重建+重启后自愈，勿当新事故。**迟到通知辨识**：被 pkill 的幻影进程完成通知（EXIT:143）可能晚到十几分钟后才弹出——收到 bg 完成通知先 `pgrep -f <script>` + tail 日志核实当前真实进程再行动
- **session 31 收尾冒烟中断点（当晚会话已闭环）**：login 200（JSON 写法，见修改规范坑③）后 quick 冒烟 0.003s 秒回疑似错误响应，读 `/tmp/q_sm.json` 核对时命令被审批 gate 拦（60s 无人点）——**当晚会话续跑完成**：quick/detail 冒烟全过（多诊断用例 1主+2次逐行+5鉴别融入、FCR 1034字）、公网验证全通、cron `4da787a0f65d` 在列（误建的重复任务 03:30 已删）、双仓提交完毕
- **⚠️ 公网登录 404 修复（session 31 收尾新发现）**：公网 `/clinical/` 302 跳 `/login`，但 nginx 只代理 `/clinical` 前缀 → `/login` 公网 404，医师端公网登录实际全断（本机直连 18790 无此问题，故本机冒烟发现不了）。修复全在服务端：`_auth_path_needs_login` 豁免 `/clinical/login|/clinical/logout`、新增这两条别名路由（复用 login_page/login_submit）、中间件 `/clinical` 重定向目标改 `/clinical/login`、登录页 JS `fetch('/login')` 改相对 `fetch('login')`（/login 与 /clinical/login 两形态自适应）、前端退出链接与 authFetch 401 跳转改 `/clinical/login|/clinical/logout`。**经验：公网可达路径 ⊂ nginx 代理前缀（/clinical、/guideline），新增对外端点必须落在前缀内或同步改 nginx**
